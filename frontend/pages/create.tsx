...
import {
  Upload, X, Plus, Minus, Mic, ChevronDown, ArrowRight, ChevronLeft, FileText,
} from 'lucide-react';
...
// ── File Upload Zone ──────────────────────────────────────────────
function FileUploadZone({ file, onFile, onClear }: {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept=".pdf,.txt,.jpg,.jpeg,.png" className="hidden"
      ...
      {file ? (
        <div className="flex items-center justify-center gap-3">
          ...
          {/* Green tick = file WILL be used for generation */}
          <div className="ml-2 flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
            Will be used
          </div>
          <button type="button" onClick={(e) => { e.stopPropagation(); onClear(); }}
            className="ml-2 w-7 h-7 bg-red-100 rounded-full flex items-center justify-center hover:bg-red-200 transition-colors flex-shrink-0">
        ...
        <>
          ...
          <p className="text-xs text-gray-400 mb-4">PDF, JPEG, PNG — up to 10 MB</p>
          <button type="button" onClick={(e) => { e.stopPropagation(); inputRef.current?.click(); }}
            className="px-5 py-1.5 bg-white border border-gray-200 rounded-full text-sm text-gray-600 hover:bg-gray-50 transition-colors shadow-sm">
            Browse Files
          </button>
        </>
      )}
      <p className="text-xs text-gray-400 mt-3">
        {file ? 'AI will read this file and create questions based on its content.' : 'Upload a PDF or image — questions will be generated from its content.'}
      </p>
    </div>
  );
}
...
export default function CreateAssignment() {
  ...
  const addQuestionType = () =>
    setFormData(prev => ({
      ...prev,
      questionTypes: [...prev.questionTypes, { type: 'Short Questions', numberOfQuestions: 1, marksPerQuestion: 1 }],
    }));

  const removeQuestionType = (index: number) =>
    setFormData(prev => ({ ...prev, questionTypes: prev.questionTypes.filter((_, i) => i !== index) }));

  const updateQT = (index: number, field: keyof QuestionType, value: string | number) =>
    setFormData(prev => {
      const u = [...prev.questionTypes];
      u[index] = { ...u[index], [field]: value };
      return { ...prev, questionTypes: u };
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.data.success) throw new Error('Failed to create assignment');
      const newAssignment = res.data.data;
      addAssignment(newAssignment);

      // ── Read uploaded file as base64 (if any) ──────────────────
      let pdfBase64: string | null = null;
      let pdfMimeType: string | null = null;

      if (uploadedFile) {
        try {
          pdfBase64 = await readFileAsBase64(uploadedFile);
          pdfMimeType = uploadedFile.type;
        } catch (fileErr) {
          console.error('Could not read uploaded file:', fileErr);
      ...
      await axios.post('/api/generate', {
        ...
        // pdfBase64 + pdfMimeType flow into AiService → PDF text extraction
        pdfBase64,
        pdfMimeType,
        formData: {
    ...
    <div className="min-h-screen bg-[#F5F5F5] flex font-sans">
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
        ...
        <div className="px-8 py-3">
          <div className="h-1.5 rounded-full bg-gray-200 overflow-hidden max-w-3xl">
            <div className="h-full rounded-full bg-gray-800 transition-all" style={{ width: '55%' }} />
          </div>
        </div>

        <main className="flex-1 px-4 pb-32 sm:px-6 lg:px-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7 max-w-3xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Science" value={formData.subject}
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. 8th" value={formData.className}
                ...
                <div className="space-y-3">
                  {formData.questionTypes.map((qt, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <button type="button" onClick={() => removeQuestionType(i)}
              ...
              <div>
                ...
                <div className="relative">
                  <textarea rows={4} value={formData.additionalInstructions}
        ...
        {/* Bottom bar */}
        <div className="fixed bottom-0 md:left-60 left-0 right-0 bg-white border-t border-gray-100 px-6 py-4 z-10">
